...
- **`config` is a real command** (`~/.local/bin/config`, installed by `config install-hooks`), so `config add ...` works in Bash tool invocations. It dispatches to `.scripts/config/config-<sub>` for `test`, `install`, `install-hooks`, `build`, `stamp`, `doctor`, `reload`, `init`, `help`, and passes every other verb to git on the bare repo. Home-relative paths still apply. `config build` builds every workspace crate and installs each stamped binary; `config doctor` reports installed binaries that no longer match their source.
...
- **One branch: `main`.** The `mac` and `linux` pair collapsed on 2026-09-06, along with `config check`, `config sync`, `config push-all`, `.sync-manifest` and the branch-drift workflow. Platform differences live in per-platform files selected at runtime (`.zshrc-mac` / `.zshrc-linux`, `tmux-mac.conf` / `tmux-linux.conf`, `deps-mac.toml` / `deps-linux.toml`), all of which ship on the single branch. `origin/mac` and `origin/linux` remain on the remote as history. `work`, `home` and `home-mac` are fossils, last touched years ago. Don't switch branches without asking.
