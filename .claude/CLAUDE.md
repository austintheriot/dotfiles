...
- **One branch: `main`.** The `mac` and `linux` pair collapsed on 2026-09-06, along with `config check`, `config sync`, `config push-all`, `.sync-manifest` and the branch-drift workflow. Platform differences live in per-platform files selected at runtime (`.zshrc-mac` / `.zshrc-linux`, `tmux-mac.conf` / `tmux-linux.conf`, `deps-mac.conf` / `deps-linux.conf`), all of which ship on the single branch. `origin/mac` and `origin/linux` remain on the remote as history. `work`, `home` and `home-mac` are fossils, last touched years ago. Don't switch branches without asking.
