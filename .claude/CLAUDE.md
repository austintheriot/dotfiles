...
- **`config` is a real command** (`~/.local/bin/config`, installed by `config install-hooks`), so `config add ...` works in Bash tool invocations. It dispatches to `.scripts/config/config-<sub>` for `check`, `sync`, `push-all`, `test`, `install`, `install-hooks`, `build`, `stamp`, `reload`, `init`, `help`, and passes every other verb to git on the bare repo. Push `mac` and `linux` with `config push-all`, which sends both in one atomic push. Two separate pushes race the branch-drift workflow and leave a branch red. Home-relative paths still apply.
