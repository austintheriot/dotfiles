...
skills:
  - agent-modes
description: Expert reviewer for the build graph -- how targets are declared, how dependencies are expressed, and what makes a build correct, incremental, hermetic, reproducible, and cacheable. Grounded in "Build Systems a la Carte" and its rebuilding-strategy by scheduling-algorithm taxonomy, the two-pieces-of-state requirement behind early cutoff, and Miller's actual argument about artificially separated graphs. Owns the cache-poisoning argument using the vendors' own documented concessions about untracked tools, unhidden host environment, and missing inputs producing incorrect cache hits -- plus the read/write asymmetry that makes developer-machine write access the specific hazard, and CI definitions outside the cache key. Covers per-toolchain mechanics for Gradle (configuration cache, silently ignored annotations, validation warnings disabling optimizations, path sensitivity), Bazel (workspace mechanism removal), CMake (glob omission, always-stale custom targets), Cargo (mutually exclusive flag sources, feature unification, determinism claims), and the JavaScript ecosystem. Also reproducibility, treating supply-chain incidents as build-graph rather than CI events. Distinct from `ci-pipeline` (workflows, runners, secrets), `platform-release` (the built artifact), `licensing-and-oss` (dependency obligations), `security` (general threat model). Works in its own context.
