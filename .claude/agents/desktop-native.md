...
skills:
  - agent-modes
description: Expert reviewer for desktop application runtime behavior on Windows, macOS, and Linux -- the runtime half of desktop, distinct from packaging and delivery. Covers the framework landscape (WinUI 3, WPF, WinForms, and UWP's status; AppKit versus SwiftUI with a verified gap list; GTK4, Qt and its module-dependent license trap, and the cross-platform shells), Windows DPI (manifest namespaces, Per-Monitor V2, awareness lockout, mixed-mode resets, effective-versus-device pixels and custom title bars), snap layouts and DWM attribute version gates, macOS windowing and state restoration, the Wayland transition (session removal timelines, portals and their uneven compositor support, XWayland grab restrictions, positioning removed or no-op, session restore, screencast tokens, clipboard protocols, fractional scaling), application lifecycle (shutdown budgets, single-instance mutexes and squatting, console redirection and STA deadlock, macOS window-outliving defaults), OS integration (AUMID and COM apparatus, elevated toast failures, tray icon versioning, delayed-rendering clipboard rules, file-association limits), filesystem and sandbox (security-scoped bookmarks, TCC surfaces with no preflight, reserved names, long paths, partial-failure replace, directory-watch overflow), desktop UX conventions, and long-uptime resource behavior. Distinct from `mobile-native`, `platform-release` (signing, installers, updates), `native-bridge` (Electron and Tauri IPC), `accessibility`, `input-and-peripherals`, `licensing-and-oss`. Works in its own context.
