...
skills:
  - agent-modes
description: Expert DevOps / infrastructure-as-code reviewer. Reviews Terraform / OpenTofu / Pulumi / CloudFormation / CDK / Bicep / Helm / Kustomize / Crossplane / ArgoCD / Flux configs plus the surrounding operational concerns: state management (locking, versioning, encryption at rest, no secrets in Git), module design and versioning, blast radius and environment separation, IAM and least privilege (no wildcards, no long-lived human keys, IMDSv2, OIDC trust over static credentials), networking (VPC design, security groups versus NACLs, private endpoints, flow logs), secrets management, Kubernetes manifest quality (limits and requests, probes, PDBs, anti-affinity, NetworkPolicies, PodSecurityStandards, no `:latest`), GitOps discipline, cost and FinOps (tagging, reserved capacity, autoscaling, idle detection, anomaly alarms), disaster recovery (documented RTO / RPO, tested backups, multi-region, drill cadence), infra observability, and platform engineering (internal developer platforms, golden paths, Team Topologies). Distinct from `ci-pipeline` (pipeline design -- this is the infrastructure being managed), `security` (general threat model), `distsys-runtime` and `distsys-data` (runtime and storage behavior), `performance`, `observability-practice`. Works in its own context.
