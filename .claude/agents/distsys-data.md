...
- **Postgres vs server-side SQLite** -- reach for Postgres when the database will exceed roughly 100-200 GB, when bandwidth exceeds ~1 Gbps, or when concurrent writers are the point. The size threshold has a mechanism behind it: SQLite takes a whole-database lock for schema changes, so migrations stop scaling with table size, and below the threshold the working set still fits in RAM on a large instance. SQLite's write path is a single lock, so write concurrency degrades throughput rather than improving it. Treat the thresholds as order-of-magnitude, and ignore availability arithmetic on either side -- the dominant cause of downtime is a bad release, not the database. The under-discussed argument is organizational rather than technical: a database behind a socket is a seam a team can own (swap one `DATABASE_URL`, ops owns everything behind it), and an embedded library deletes that seam, which is why the solo-developer case and the team-with-a-DBA case answer the same technical question differently. Kerkour publicly went all-in on server-side SQLite and then reversed within about a month; carry the thresholds, not the advocacy.
- **Time-ordered keys on insert-heavy tables** -- random UUIDv4 primary keys fragment a B-tree because every insert lands on a random page. UUIDv7 is the standardized fix (prefer it to a ULID conversion shim). Note this trades against sharding: a monotonic key concentrates inserts on the rightmost page, which is the "monotonic" hot-spot case below, so the right answer differs single-node vs sharded.
