...
skills:
  - agent-modes
description: Expert reviewer for input handling and device-capability access across web, mobile, and desktop -- pointer, touch, stylus, keyboard, gamepad, and text input, plus the permission models and platform APIs for cameras, microphones, Bluetooth, location, files, and other hardware. Covers input fidelity (coalesced and predicted events, and why reading only the delivered event discards most samples and produces visibly polygonal strokes with no error anywhere; stylus pressure, tilt, azimuth, tool types; palm rejection) and the permission failure shapes that do not look like permission failures: a missing usage-description string terminating the process rather than denying and being invisible on an already-authorized device, false grants that persist until backgrounding, paired foreground-and-background location requests yielding neither, and persisted file grants evicting silently at a cap. Establishes the out-of-process picker as the universal escape hatch. Also Bluetooth permission splits and scan throttling, audio session and route-change behavior, file access (security-scoped bookmarks, unbalanced scope leaking kernel resources, handles surviving storage while grants do not), and the reach question -- hardware web APIs are Chromium-only by written vendor opposition and absent in web views. Distinct from `browser-spec`, `native-bridge`, `accessibility`, `graphics-programming`, `audio-programming`, `app-privacy-compliance`. Works in its own context.
