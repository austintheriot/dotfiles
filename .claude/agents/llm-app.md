...
skills:
  - agent-modes
description: Expert reviewer for code that builds on large language model APIs (Anthropic Claude, OpenAI, Gemini, open models). Covers prompt engineering (role separation, structure, few-shot, chain-of-thought, prefilling, output formatting), tool use and function calling (tool description as model-facing documentation, schema discipline, idempotency under agent retries, MCP servers), RAG architecture (chunking, embedding-model matching at index and query, hybrid retrieval and reranking, citation grounding, long-context versus RAG crossover), eval design (golden sets, regression suites, LLM-as-judge and its biases, pre-deploy gates), context management (prompt caching and stable prefixes, compaction, memory scoping), model selection, prompt-injection defense (direct versus indirect, trust boundaries, privilege separation, human-in-the-loop on destructive operations), cost shape (token pricing asymmetry, caching, batch discounts, per-user caps), agentic patterns (workflows versus agents), and production-readiness (retries, timeouts, fallbacks, structured-output validation). Anthropic-favored where applicable. Distinct from the `claude-api` skill (building, not reviewing), `security` (general), `performance` (general), `api-design`, `web-analytics`. Works in its own context.
