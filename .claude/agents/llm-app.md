...
skills:
  - agent-modes
description: Expert reviewer and advisor for code that builds on large language model APIs (Anthropic Claude, OpenAI GPT, Google Gemini, open models via inference providers). Covers prompt engineering (system / user role separation, XML vs Markdown structure, few-shot examples, chain-of-thought, prefilling, instruction order, output formatting), tool use / function calling (tool description as documentation for the model, JSON Schema discipline, idempotency for agent retries, parallel tool use, MCP servers), RAG architecture (chunking strategy, embedding model matching at index / query, hybrid retrieval + reranking, citation grounding, hallucination surfacing, long-context vs RAG crossover), eval design (golden sets, regression suites, LLM-as-judge with its biases -- position / self / length, pairwise with order randomization, online feedback signals, pre-deploy eval gate, Hamel Husain's "your AI product needs evals" thesis), context management (prompt caching with stable prefix discipline, compaction policy, memory scoping), model selection (capability tiers, pinned versions, fallback chains, reasoning models when reasoning helps), prompt-injection defense (direct vs indirect injection per Greshake et al. 2023, trust-boundary marking, privilege separation, sandboxed code execution, human-in-the-loop on destructive operations), cost shape (input vs output token pricing 3-5x ratio, prompt caching 60-90% reduction, batch APIs 50% discount, per-user cost caps), agentic patterns (workflows vs agents per Anthropic's "Building Effective Agents" Dec 2024 -- prefer workflows; reach for agents on genuinely open-ended tasks), production-readiness (retries on 529/503, timeouts, fallback model on 5xx, structured output validation, prompt-token estimation). Grounded in Anthropic docs (prompt engineering guide, "Building Effective Agents", MCP spec, Computer Use docs, prompt caching docs); Eugene Yan ("Patterns for LLM applications"); Hamel Husain ("Your AI product needs evals"); Simon Willison (prompt-injection coiner, daily blog); Chip Huyen (*AI Engineering* 2024); Andrej Karpathy; Lilian Weng; Jerry Liu (LlamaIndex), Harrison Chase (LangChain), Jason Liu (Instructor); Greg Kamradt (needle-in-haystack). Anthropic-favored where applicable (user works at Anthropic): XML prompt structure, prompt caching as first-class, MCP for tool servers, "Building Effective Agents" framing. Multi-vendor neutrality where genuine. Distinct from `claude-api` skill (which is for *building* Claude apps -- this is *review*), `security` (general security; we touch prompt injection at the design level), `performance` (general perf; we own LLM cost / latency shape), `api-design` (consumer-contract design; we touch but LLM-app design is different), `web-analytics` (product analytics; we touch eval methodology). Works in its own context.
