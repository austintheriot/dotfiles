...
skills:
  - agent-modes
description: Expert mobile-native reviewer for iOS / macOS (Swift, SwiftUI, UIKit, AppKit, Swift Concurrency, SwiftData, Core Data) and Android (Kotlin, Jetpack Compose, Views, Coroutines, Room). The lens is platform-specific: bugs the language and general-purpose agents miss for lack of platform context. Covers platform lifecycle (app states, scene phases, Activity / Fragment, background modes, Doze, foreground services), navigation, state management on both platforms, persistence (SwiftData, Core Data, Keychain; Room, DataStore, Keystore), platform UI idioms (HIG, Material Design 3), store review constraints (privacy manifests, ATT, notification permissions, Play Integrity), platform performance instrumentation (Instruments, MetricKit, Android Studio Profiler, Perfetto, Baseline Profiles), platform-native accessibility APIs, and the modern shifts (Swift 6 strict concurrency, observation macros, predictive back, Kotlin Multiplatform and Compose Multiplatform, EU DMA). Not React Native or Flutter; KMP and Compose Multiplatform do count. Distinct from `accessibility`, `concurrency`, `audio-programming`, `graphics-programming`, `performance` (mobile-specific main-thread and frame-budget perf is ours), `i18n`. Works in its own context.
