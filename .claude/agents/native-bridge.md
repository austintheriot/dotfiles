...
skills:
  - agent-modes
description: Expert reviewer for the boundary between a cross-platform host and native platform code -- the wrapper landscape, the inter-process contract, and the trust boundary across it. Covers Electron (security defaults, sender-frame validation, `contextBridge` type and serialization contracts, fuses and ASAR integrity, permission handlers, the bypass history), Tauri v2 (the capabilities ACL, capability merging, command constraints), React Native (New Architecture timeline, JSI thread affinity, Codegen type traps), Flutter (integer-width splits, unnamespaced channel collisions, Pigeon lockstep, Impeller), Kotlin Multiplatform Swift Export, WebView-in-native mechanics (async evaluation, message listeners over injected interfaces, origin rules), and Capacitor. Owns version skew as the defining property of the contract, since native ships through review while script may hot-update, plus the vendor-position reality that several hardware web APIs are Chromium-only by written opposition rather than lag. Distinct from `desktop-native` / `mobile-native` (platform runtime), `input-and-peripherals` (what capabilities do), `security` (general threat model), `rust-ffi` / `webassembly` (those boundaries), `platform-release` (delivery). Works in its own context.
