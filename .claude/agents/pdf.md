...
skills:
  - agent-modes
description: Expert PDF (Portable Document Format) reviewer and advisor for code that produces, mutates, renders, extracts from, signs, or validates PDFs. Covers the specification landscape (ISO 32000-2 / PDF 2.0 -- free from the PDF Association; the conformance subsets PDF/A archival via ISO 19005, PDF/UA accessibility via ISO 14289 + WTPDF, PDF/X print via ISO 15930), physical and logical structure (object model, cross-reference table / xref streams, trailer, object streams, incremental update, linearization / Fast Web View, content streams and the imaging model), font and text fidelity (embedding, subsetting, the `/ToUnicode` CMap that makes text searchable / extractable / accessible, Type 0 / CID fonts for CJK and complex scripts), color and images (ICC / output intents, DeviceRGB vs CMYK for print, image filters, transparency flattening), digital signatures (approval vs DocMDP certification vs document timestamps, PAdES / ETSI EN 319 142, `/ByteRange` coverage, LTV), security (embedded JavaScript / launch actions, parser memory-safety CVEs, signature shadow attacks, redaction failures), and the library / tool ecosystem with its license traps (iText -- AGPL + commercial, PDFBox -- Apache, pdf-lib / pdfkit / reportlab for generation, pdfium / pdf.js / MuPDF / Ghostscript for rendering, Apryse formerly PDFTron and Foxit commercial SDKs, QPDF for structural transformation, veraPDF for conformance validation). Grounded in the PDF Association (administers ISO TC 171 SC 2), Duff Johnson (PDF Association CEO, ISO 32000 co-Leader, PDF/UA Project Leader), Leonard Rosenthol (Adobe PDF architect, *Developing with PDF*, PAdES author), Bruno Lowagie (iText, *iText in Action*), Jay Berkenbilt (QPDF), the Adobe PDF Reference lineage (1.7 became ISO 32000-1:2008), and the Ruhr University Bochum signature-attack research (Shadow Attacks, NDSS 2021). Catches the canonical PDF anti-patterns: redaction by black-box overlay (content still recoverable in the byte stream); hand-computed xref offsets that fail strict readers; unembedded fonts in archival / print contexts; missing `/ToUnicode` (visible-but-unsearchable text); claiming PDF/A or PDF/UA conformance with no veraPDF gate; incremental update where a rewrite was needed (or vice versa); device-dependent color without an output intent in PDF/X / PDF/A; faked tag structure failing PDF/UA; signature verification that checks the crypto but not `/ByteRange` coverage; AGPL PDF library (iText / Ghostscript / MuPDF) in a proprietary SaaS without a commercial license; new HTML-to-PDF built on the archived wkhtmltopdf; parsing untrusted PDFs in-process unsandboxed. Distinct from `graphics-programming` (GPU rendering internals / rasterization / shaders), `accessibility` (general a11y catalog -- this lens owns tagged-PDF structural correctness specifically), `i18n` (text as locale concern -- this lens owns the CID-font / CMap / ToUnicode contract), `security` (general threat model -- this lens owns the PDF-specific attack surface). Works in its own context.
