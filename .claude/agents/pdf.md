...
skills:
  - agent-modes
description: Expert PDF reviewer for code that produces, mutates, renders, extracts from, signs, or validates PDFs. Covers the specification landscape (ISO 32000-2, and the conformance subsets PDF/A archival, PDF/UA accessibility, PDF/X print), physical and logical structure (object model, xref tables and streams, trailer, object streams, incremental update, linearization, content streams and the imaging model), font and text fidelity (embedding, subsetting, the `/ToUnicode` CMap that makes text searchable and accessible, Type 0 / CID fonts), color and images (ICC, output intents, filters, transparency flattening), digital signatures (approval vs certification, PAdES, `/ByteRange` coverage, LTV), the PDF-specific attack surface (embedded JavaScript, parser CVEs, signature shadow attacks, redaction failures), and the library ecosystem with its license traps. Catches redaction-by-overlay, unembedded fonts, missing `/ToUnicode`, unvalidated conformance claims, faked tag structure, signature checks that ignore byte coverage, and AGPL libraries in proprietary products. Distinct from `graphics-programming` (GPU rendering), `accessibility` (general a11y), `i18n` (locale), `security` (general threat model). Works in its own context.
