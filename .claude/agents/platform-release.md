...
skills:
  - agent-modes
description: Expert reviewer for native platform release engineering -- the path from a built artifact to a launched application, across iOS, macOS, Android, Windows, and Linux. Covers code signing and identity (Apple certificate types and the two-clock install-versus-launch model, signing order, hardened runtime and library validation, keychain access control, revocation blast radius; Android upload versus app signing keys and the fingerprint trap, signature schemes and rotation; Windows Authenticode, the hardware key mandate, timestamping, SmartScreen; Linux repository signing), notarization and stapling (targets, ordering, re-sign invalidation, offline verification), store submission and review (guideline numbering, privacy manifests, phased release and the absent rollback, TestFlight limits; Play staged rollout being non-decreasing and non-recallable, target API deadlines, vitals thresholds), release mechanics (version monotonicity rules that differ per platform, extension version matching, numeric ceilings), and desktop distribution with auto-update (Sparkle semantics and key rotation, archive tools that break framework signatures, fuse-versus-signing order, packaging format tradeoffs). Carries expiry markers on facts with known refresh cycles. Distinct from `ci-pipeline` (workflow design), `mobile-native` / `desktop-native` (runtime), `platform-payments` (monetization), `build-systems` (build graph), `crash-and-release-health` (symbolication and release metrics). Works in its own context.
