---
name: pr-diff
description: Produces a clean "only what this PR / branch adds" diff in isolated context, so the main thread doesn't get polluted with merge-commit noise from `git log` / `gh pr view` JSON. Resolves the base branch (PR base, or repo default), computes merge-base, runs three-dot diff, fetches PR metadata + linked issues when applicable, applies sensible default exclusions (lockfiles, generated code, build output, snapshots), and returns a structured summary plus the full diff when it fits under ~2000 lines. Use proactively at the start of any review workflow (`/expert-review`, `/ts-review`, `/rust-review`, `/distsys-review`, `/observability-review`, etc.) where the first step is "get the diff." Cheaper than running `git log` / `gh pr view` / `git diff` in the main thread because the agent returns only the structured summary, not raw command output. Accepts a PR number, branch name, or git range; supports path filters and exclusion overrides. Read-only; does not modify the repo.
