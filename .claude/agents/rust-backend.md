...
**Local error types must not cross a service boundary.** The tiering above is by intent (enum when callers branch, opaque when they only report); this one is by architecture and composes with it. Each service keeps a fine-grained internal `Error`, but its public methods return a shared coarse type whose variants are roughly the gRPC status set (`NotFound`, `InvalidArgument`, `PermissionDenied`, `AlreadyExists`, `AuthenticationRequired`, `Internal`), with `impl From<LocalError> for SharedError` doing the mapping. A caller then propagates with `?` and never learns that `users::Error::SessionIsNotValid` exists. *Review flag:* a service's local error type appearing in another service's signature -- that is a leak that welds the two together.

**Log the `Internal` variant, not the 4xx ones.** Centralized error handling maps variant to status; only the server-fault branch logs at error level. *Why:* an error-level log per 404 buries real faults in client noise. *Review flag:* a handler logging every validation failure at `error!`.
...
**Batch writes, and know the 65,535 bind-parameter ceiling.** One statement beats a loop: `INSERT ... VALUES` for many rows, or `UPDATE t SET col = v.col FROM (VALUES ...) AS v(id, col) WHERE t.id = v.id` for many updates. Kerkour measured 300-500k upserts dropping from roughly 30 minutes to about 200 seconds this way (his hardware, order of magnitude only). *The ceiling is a correctness issue, not a tuning knob:* Postgres rejects a query above 65,535 bind parameters at runtime, so a `sqlx::QueryBuilder::push_values` batch must chunk at `65535 / columns_per_row`. Code that passes staging at 10k rows fails in production at 40k. *Prefer `unnest` for large batches:* `INSERT INTO t SELECT * FROM unnest($1::uuid[], $2::text[])` takes one array parameter per column regardless of row count, so the chunking loop disappears and the query text is constant -- `push_values` emits new SQL for every distinct chunk size, which churns the prepared-statement cache. *Review flag:* a `push_values` batch with no chunk bound; row-at-a-time writes inside one transaction mistaken for batching.

**If the service uses SQLite instead, two settings and one topology carry most of the risk.** Enable WAL mode (readers and the writer stop blocking each other) and set a `busy_timeout`. Then know the trap that `busy_timeout` does not cover: SQLite transactions default to `DEFERRED`, meaning read-only until the first write, and **a deferred transaction that upgrades to a write while the database is locked returns `SQLITE_BUSY` immediately without honoring `busy_timeout`**. Start anything that will write with `BEGIN IMMEDIATE`. Topology: one pool with `max_connections = 1` for writes and a separate read pool, both against the same file. That turns SQLite's retry-and-fail lock into an application-side queue and removes `SQLITE_BUSY` rather than making it rarer. *Consequence to state explicitly:* a wrapper routing by method name is a contract, not a convenience -- a read sent through the write path silently serializes, and a read-your-own-writes read sent through the read pool will not see the open write transaction. *Counterintuitive and worth flagging:* raising write concurrency makes SQLite writes **slower** (Kerkour measured roughly 15.3k inserts/s at concurrency 3 falling to about 9.75k at concurrency 100, his hardware, order of magnitude only), because contention on the single write lock exceeds any gain. *Review flag:* a pool size raised to fix slow SQLite writes; `BEGIN` where `BEGIN IMMEDIATE` belongs.
...
**Cron jobs need leader election the moment you run more than one replica.** Ten replicas each running an in-process scheduler fire every job ten times. If you already run Postgres, a session-scoped advisory lock (`pg_try_advisory_lock`) is the cheapest correct answer: exactly one replica acquires it, and the lock releases automatically when that connection drops, so a crashed leader needs no timeout logic or manual cleanup. Losers exit rather than spin. *Why:* it reuses a dependency you already operate instead of adding etcd or ZooKeeper for one scheduler. *Caveat:* it binds leadership to a connection, so a pooler in transaction mode will break it -- hold a dedicated connection for the lock, which in practice means a second direct-to-Postgres pool (a `DIRECT_URL` alongside the pooled `DATABASE_URL`) sized to the number of services taking a lock, often just two. This buys single-firing, not exactly-once; a leader can still die mid-job, so jobs stay idempotent. *Review flag:* a scheduler started unconditionally in every replica; leader election by "replica 0 does cron" when the orchestrator can run two; the advisory-lock connection returned to the pool while still holding leadership.
...
**Self-sandbox after load, on Linux.** Once config, certificates, and secrets are loaded, the process can drop to its minimum surface with `landlock`: read-only `/usr`, `/etc`, `/dev`, outbound TCP restricted to the ports it actually uses, inbound bind to the one listening port. The ordering is the load-bearing part -- restrict *after* load, not at startup. The stronger design consequence is upstream: **build services that never touch `/tmp`** (work in memory or write straight to object storage), which makes both the sandbox policy and a `FROM scratch` image trivial. *Caveat:* Landlock is Linux-only and kernel-gated, and the obvious implementation logs `NotEnforced` and continues, which is fail-open. A service that believes it is sandboxed and is not is worse than one that knows it is not, so log enforcement status at startup and treat a downgrade as fatal in a hardened deployment. *Review flag:* a sandbox call whose return status is discarded.
