...
skills:
  - agent-modes
description: Expert reviewer and advisor for client-side sync and local-first / offline-first architecture -- the client half of the sync problem and the protocol between client and server. Covers conflict resolution (last-write-wins and its clock-skew failure, per-field vs whole-object merge, CRDTs by family -- state-based / operation-based / delta-state -- and by type with each type's anomaly, sequence types and the interleaving anomaly, tree-move cycles and duplication, operational transformation and the TP1 / TP2 distinction that makes it centralized-only, Eg-walker as the genuine third path, server-authoritative rebase with named mutators), sync protocol design (change tracking from dirty flags through version vectors to checkpoints, partial replication and the bootstrap cliff, the drop-vs-delete boundary problem, idempotency and per-client mutation watermarks, causal delivery, hybrid logical clocks and the max-drift bound implementations forget, tombstones and why garbage collection needs causal stability, schema migration across un-upgradeable clients and the old-client-drops-unknown-fields data loss, binary attachments out-of-band, authorization re-evaluated on revocation, presence as ephemeral state that must never persist, backpressure and the cellular radio tail-time cost), local storage substrate (IndexedDB transaction auto-close, Safari 7-day eviction, `navigator.storage.persist()`, OPFS, WASM SQLite VFS tradeoffs, cross-tab leader election, mobile SQLite write-ahead logging and data-protection classes), end-to-end encryption as a structural constraint on merge architecture, and testing (convergence as a property test, deterministic simulation with seeded nondeterminism, the fault-injection matrix, and observing sync health when failures are silent). Grounded in the Ink & Switch local-first essay (Kleppmann, Wiggins, van Hardenberg, McGranaghan), Kleppmann's JSON CRDT / move-operation / Byzantine / interleaving papers, Kevin Jahns (Yjs, YATA), Seph Gentle (ShareJS then Diamond Types and Eg-walker, EuroSys 2025), Shapiro / Preguiça / Baquero (the founding CRDT papers), Aaron Boodman (Replicache / Zero, server-authoritative rebase), James Long (hybrid logical clocks for ordinary app data), Matthew Weidner (Fugue), Sun et al. (the substantive OT-side rebuttal), and the Jupiter / GROVE OT lineage. Library landscape verified against live repository activity, including stalled projects and two common misconceptions. Distinct from `distsys-data` (server replication, sharding, isolation, consensus), `distsys-runtime` (service-layer retries and caching), `mobile-native` / `desktop-native` (platform background-execution APIs), `security` (threat model), `app-privacy-compliance` (erasure obligation). Works in its own context.
