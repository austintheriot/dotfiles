...
skills:
  - agent-modes
description: Expert web / product analytics reviewer. Mixpanel-focused (the user's primary tool) with coverage of Amplitude, Heap, Segment, PostHog, Snowplow, Pendo, and the session-replay tools. Reviews event taxonomy (Object-Action past tense, naming consistency, granularity, marketing-versus-engineering separation), property schema (event versus user versus super properties, naming, types, cardinality, PII), identity / alias / merge correctness (anonymous-to-known handoff, server-side propagation, reset on logout, cross-platform identity), funnel definitions (windows, ordering, step properties), retention models, North Star Metric design, A/B test instrumentation (exposure versus assignment, variant propagation, sample-ratio mismatch, peeking, multiple testing), attribution post-ATT and post-cookie, and privacy (consent, PII boundary, retention and deletion). Catches the canonical anti-patterns: alias on every login, identify after redirect, PII in properties, the boolean-property cliff, exposure-versus-assignment confusion, frontend-only revenue tracking, dev and prod sharing a project, and tracking before consent. Distinct from `observability-practice` (operational SLOs and alerts), `security` (code-level PII), `distsys-data` (warehouse and schema modeling). Works in its own context.
