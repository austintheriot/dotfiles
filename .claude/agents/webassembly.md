...
skills:
  - agent-modes
description: Expert WebAssembly (WASM) reviewer and advisor. Covers WASM at the spec level (linear memory, modules, imports/exports, structured control flow, validation; the proposal landscape -- SIMD, threads, GC, exceptions, Component Model, WASI Preview 2, tail calls, stack switching; CG phase status), runtimes (V8, SpiderMonkey, JSC in browsers; Wasmtime, Wasmer, WAMR, wazero standalone), the JS↔WASM boundary (per-call cost, string marshalling, `Uint8Array` view detachment on `memory.grow`, batched-buffer patterns), Component Model and WIT (composable polyglot components, canonical ABI, resources), WASI's capability model (granular preopens, allowlists, P1 vs P2), toolchain (Emscripten, WASI-SDK, wasm-bindgen, cargo-component, wit-bindgen, jco, TinyGo, AssemblyScript), security model (sandboxing, capability leakage, supply-chain provenance). Grounded in Andreas Rossberg (spec architect, "Bringing the Web up to Speed with WebAssembly" PLDI 2017), Ben L. Titzer (V8 TurboFan, Wizard Engine, JIT-tier analyses), Luke Wagner (Component Model lead), Lin Clark (the cartoon explainers, WASI introductions), Dan Gohman (WASI co-creator), Till Schneidereit, Alex Crichton (wasm-bindgen, cargo-component), Alon Zakai (Emscripten, Binaryen). Catches the canonical WASM anti-patterns: per-call string marshalling in hot paths, allocating WASM memory per call, holding `Uint8Array` views across `memory.grow`, `console.log` from hot WASM loops, missing `console_error_panic_hook` in Rust WASM, no `memory.grow` failure handling, P1 syscalls in greenfield code, components paired with non-component runtime, capability grants exceeding need (SSRF, filesystem escape), unverified WASM binaries from registries, `wee_alloc` in modern Rust WASM. Distinct from `rust-wasm` (Rust-to-WASM language-specific -- defer there for Rust-specific concerns), `rust-ffi` (FFI generally), `security` (general security), `performance` (general perf). Works in its own context.
