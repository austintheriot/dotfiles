...
**The reference implementation states the problem better than any critique could.** blender-mcp added an opt-in abstract-syntax-tree allowlist "safe mode" in September 2026, and its own docstring is precise about what that does and does not achieve: the Blender addon's socket "accepts a raw `execute_code` from **any local process** -- it is a guard on what the model can be talked into, not a sandbox around Blender." Three facts compound it: **safe mode is off by default**; the MCP specification's own Local Server Compromise guidance says such servers SHOULD use stdio or a Unix socket with an auth token, and **neither is implemented**; and the documented install path is unpinned against a repository with **no releases or tags**. Any listening local socket that executes code is reachable by every process on the machine, not only by the agent -- which is a different and larger threat model than "the model might do something wrong."
...
Two thresholds that are widely misquoted and that determine whether a licence binds you: **Hunyuan3D's monthly-active-user threshold is 1 million, not the 100 million figure that circulates** (that belongs to other Chinese model licences). And **Stability's $1M cap is whole-company revenue, not AI-attributable revenue** -- the text says "regardless of whether that revenue is generated directly or indirectly." A studio well under a million dollars of AI-derived revenue can still be over the cap.
...
**Source research**: `~/.claude/local/research-notes/ai-creative-research.md` (claims tagged verified / found / inferred, with an explicit gaps appendix). Read it before a refresh -- it records what was verified against a primary source, what was not, and which sites blocked automated fetching, so a refresh pass need not re-derive any of that.

- **2026-09-05** -- Initial version. MCP facts verified against the specification and its changelog at revision `2026-07-28`, including the removal of sessions and the initialize handshake, the `server/discover` requirement, `subscriptions/listen`, the loss of stream resumability, MRTR, and the deprecation of Roots, Sampling, and Logging. Blender failure modes verified against Blender's own Python API documentation, including the undo-invalidation warning and its "use at your own risk" nuance, the operator poll-failure diagnosis problem, and the name-assignment caveat. Perception-loop token arithmetic uses the documented image-token approximation. blender-mcp's safe-mode docstring, its default-off posture, and the absence of the stdio-or-authenticated-socket measure the MCP spec recommends were read from the repository directly. Hunyuan3D's 1-million-MAU threshold and Stability's whole-company revenue cap were read from the licence texts and correct figures that circulate wrongly. The legal section is deliberately thin and points at primary sources: it is the fastest-rotting and most jurisdiction-dependent material here, and it is not legal advice.
