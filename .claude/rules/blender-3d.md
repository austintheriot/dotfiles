...
- **The `bpy` PyPI wheel** ("Blender as a Python module") lets Blender run inside a normal Python process, for pipelines and services. **Each Blender release supports exactly one Python version** -- the 5.2.1 wheel requires Python 3.13 exactly. This is a hard continuous-integration constraint: the runner's Python must match the wheel's Blender line, and a Blender upgrade forces a Python upgrade in lockstep.
