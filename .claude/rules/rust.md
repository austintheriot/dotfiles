...
- **Constructors are inherent: `new`, `with_<detail>`, `from_<source>`.** For optional knobs, the choice is by boundary, not by param count. *Crate-internal and application types:* a config struct with `Default` plus functional update (`Config { retries: 3, ..Default::default() }`) beats a builder -- the boilerplate is written once instead of growing per setting, and go-to-definition on `default` shows every default in one place instead of scattering them across `new` and the setters. *Published API boundaries:* builders still earn their keep, which is why `reqwest`, `tokio`, `hyper` and `clap` all use one. Adding a method is never breaking; adding a public field breaks every exhaustive struct literal and every destructure. `#[non_exhaustive]` is the bridge and forces callers onto `..Default::default()`. Builders are also the only option when a field is genuinely required (`Default` demands a sensible zero for everything, and an `Option<T>` stand-in pushes the invariant back to runtime) or when `impl Into<String>` setter ergonomics matter.
