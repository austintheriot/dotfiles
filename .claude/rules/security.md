...
1. **What are the assets?** Data (user PII, payment data, secrets, intellectual property), capability (admin endpoints, privileged operations), availability (the service itself), **and the credit card** -- on usage-priced infrastructure, cost scales with traffic rather than revenue, so the bill is an asset an anonymous attacker can attack directly (denial of wallet). One commodity box saturating a 1 MB endpoint extrapolates to five- and six-figure monthly egress on several popular platforms, and a serverless stack bills the same request at the CDN, the gateway, and the function. *The control is a hard ceiling, not a rate limit:* budget alerts with automated actions, concurrency caps, per-account quotas, and caching so origin egress stays bounded. *Review flag:* name the endpoint, name the metered unit (requests, GB egress, GB-seconds, LLM tokens), then ask what stops the meter -- "nothing" is the finding. Note the loudest public incidents are self-inflicted loops rather than attacks, which strengthens the no-ceiling point.
...
- *The slow-KDF rule is about entropy, not about the word "credential"*: a **user-chosen** password needs a slow KDF because it is guessable. A **32-byte CSPRNG** API key or session token has nothing to brute-force, so a single fast hash (SHA-512 / SHA3-512, sub-microsecond) is correct, and Argon2id there is an unauthenticated DoS vector -- every failed request costs ~100ms of CPU and tens of MB. Do not file "API keys are not Argon2-hashed" as a finding unless the secret is user-chosen or low-entropy.
- *Bind the hash preimage to the row's identity*: hashing the secret alone lets an attacker with database **write** access overwrite a victim's `secret_hash` with the hash of a key they legitimately own, then authenticate as the victim. Hashing `id || version || owner_id || secret` makes the swapped row fail verification. This is a distinct threat from the request-forwarding confused deputy above: the adversary here has DB access but no code access.
...
- *Bind the algorithm to the key, not to an allowlist*: store `{kid, alg, secret}` together so a token claiming a different `alg` for that `kid` fails structurally, instead of relying on a verify-side allowlist someone forgets. Rotation falls out of the same shape: config holds an **array** of keys, sign with the newest, accept the previous one briefly, keep at most two. Prefer EdDSA or ES256 over RS256 (RSA's padding and key-size footguns), though correctly-implemented RS256 is what most OIDC providers emit and is not itself a finding.
- *Scope a token to its action*: include an `action` claim so a `verify_email` token cannot be replayed against `promote_to_admin`. To make a token single-use **without server-side state**, embed a value the action itself mutates -- `old_email` for an email change, the user's `updated_at` for a password reset -- so the token stops verifying once it has been used.
- *Login timing leaks account existence*: if the expensive hash runs only when the user row was found, a sub-100ms response means "no such account", regardless of how generic the error text is. The fix is **constant work** -- hash against a fixed dummy hash on the not-found branch. A random sleep adds noise but does not remove the signal, and averaging over enough requests recovers it.
- *Passkeys beside passwords are a feature, not a control*: if the password still works, the phishing surface is unchanged. A real migration removes the password and falls back to a magic link or one-time code. Prefer COSE `-8` (Ed25519) and `-7` (ES256); offer legacy devices another path rather than accepting `-257` (RS256). The WebAuthn PRF extension yields a per-relying-party symmetric key, which is how end-to-end encryption gets built on passkeys without a second password-derived key. *Caveat:* deleting stored passwords collides with enterprise SSO, recovery, and support workflows -- carry the insight, not the prescription.
...
- **CWE-352 CSRF** -- state-changing operations via GET, or POST without anti-CSRF token. SameSite cookies are the modern primary defense. A JSON API is *mostly* immune for a precise reason: an HTML form can only emit `application/x-www-form-urlencoded`, `multipart/form-data`, or `text/plain`. That immunity evaporates if the server does not actually enforce `Content-Type: application/json`, if `Access-Control-Allow-Origin: *` is set, or if cookie auth is paired with a permissive CORS policy. Enforcing the content type server-side is the cheap check to look for.
...
- *Dependency code runs before you run anything*: in Rust, `build.rs` and proc-macros are arbitrary code executed during `cargo check`, not just `cargo build`. rust-analyzer runs both when it loads a project, so **opening a folder in an editor is enough to execute a transitive dependency's code** -- no build, no test, no run. The npm `postinstall` analogue fires at install time; this one fires at editor-open time. Inspect with `https://docs.rs/crate/<CRATE>/<VERSION>/source/`. *Review flag:* a diff adding a `[build-dependencies]` entry or a new proc-macro crate -- that is code that will run on every contributor's laptop.
- *A published package is not its repository*: `cargo publish --allow-dirty` ships the uncommitted working tree, and nothing verifies published contents against the git tag. Analysis of the top ~1000 crates found roughly one in six (17-20%, Adam Harvey) containing code that does not match their repository. So "I read the crate's GitHub source and it looked fine" establishes nothing about what you compiled. Countermeasures: `cargo vendor` with diff review on update, or `[patch.crates-io]` pinned to a git **`rev` SHA** -- never a branch, which is mutable and force-pushable and therefore strictly worse than an immutable published version. Worth the effort on low-popularity dependencies, where a compromise goes unnoticed longest.
- *Human-audit provenance, not just CVE matching*: `cargo-vet` (distinct from `cargo audit`) records which dependencies a trusted party has actually read. Google (`google/rust-crate-audits`) and Mozilla (`mozilla/supply-chain`) publish theirs in an importable format, so a small team inherits audit coverage instead of producing it.
- *The auto-update path is itself a delivery channel*: a stolen publish token plus a patch bump (1.12.0 -> 1.12.1) lands via semver ranges with no manifest change and no human decision. Exact pinning defeats it but also blocks security patches -- a real trade, not a free win. *Review flag:* a lockfile diff that is only patch bumps, arriving outside a normal dependency-maintenance PR.
