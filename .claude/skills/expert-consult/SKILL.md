...
Eligible agents are the consult-capable ones from `/consult`. Run `ls ~/.claude/agents/` at session start to pick up new ones.
...
- **`security`**, **`performance`**, **`accessibility`**, **`api-design`**, **`concurrency`**, **`i18n`**, **`ci-pipeline`**, **`devops-infrastructure`**, **`graphics-programming`**, **`audio-programming`**, **`pdf`**, **`citation-and-bibliography`**, **`scholarly-publishing`**, **`copyright-and-permissions`**, **`print-production`**, **`webassembly`**, **`mobile-native`**, **`sync-and-offline`**, **`platform-payments`**, **`platform-release`**, **`desktop-native`**, **`app-privacy-compliance`**, **`native-bridge`**, **`input-and-peripherals`**, **`licensing-and-oss`**, **`build-systems`**, **`crash-and-release-health`**, **`llm-app`**, **`ai-creative-tooling`**, **`ai-3d-integration`**, **`browser-spec`**, **`web-analytics`**, **`data-flow`**, **`neovim`**, **`blender-3d`**, **`game-art-pipeline`**, **`level-design`**, **`game-engines`**, **`game-mechanics`**, **`architectural-design`**, **`interior-and-spatial`**, **`architectural-tooling`**, **`agent-sandboxing`**, **`agent-orchestration`**, **`local-inference`**.
...
| "How do I structure my agent / RAG / eval system" | `llm-app` + `agent-orchestration` (if several agents run at once) + `observability-practice` + (maybe `security` for prompt injection) |
| "How many agents can I run at once / why did my parallel agents stall or race" | `agent-orchestration` + (maybe `local-inference` if they share a local model server) |
| "Is my agent sandbox actually enforced / what can the agent reach" | `agent-sandboxing` + `security` |
| "Should I run this model locally, and on what" | `local-inference` + `llm-app` + (maybe `product-leadership` for the cost decision) |
