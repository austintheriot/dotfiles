...
- **`interaction-design`** -- state completeness at spec time: does the spec say what every surface does when empty, loading, partial, failed, or unauthorized; is every irreversible operation paired with undo or proportional confirmation; are feedback and validation timing specified. **Silence on a state is a spec defect**, and it is much cheaper to name here than after the component exists. Fires whenever the spec describes a UI surface.
- **`information-architecture`** -- the page inventory, the category scheme, and above all the vocabulary. A spec that names its objects inconsistently produces a product that does, and renaming after implementation is expensive. Fires whenever the spec introduces a nav structure, a page set, or domain terms.
- **`design-systems`** -- whether the spec commits to a token layer and a component contract before components exist. The cheapest moment to decide the semantic tier is before anything references a primitive. Fires when the spec describes UI at all, and especially when it leaves the visual system as an open question.
- **`ai-interface-design`** -- fires when the spec puts model output in front of a person. Owns whether the spec has decided what confidence means, what a person sees before deciding, what provenance is recorded, and what an agent may do unattended. Specs routinely settle the data model and leave every one of these open.
- **`expert-user-efficiency`** -- fires only when the spec describes a high-frequency working surface (a queue, a grid, an editor, an operational tool) and says or implies volume. Owns whether the spec's workflow is affordable at its own stated scale. Skip for first-use and occasional surfaces.
- **`content-design`** -- fires when the spec specifies user-facing strings, error taxonomies, or status vocabularies. Lower priority than the above at spec time; most content work is better done against a real surface.
- **`visual-hierarchy`** -- rarely useful at spec time, since a spec has no pixels. Consult opportunistically only if the spec makes a claim about density, layout, or a type or colour system.
...
- **`game-mechanics`** -- fires when the spec describes gameplay systems. Owns whether the loop's incentives match its stated goal, whether feedback loops are signed deliberately, and whether an economy conserves. Incentive misalignment is far cheaper to name at spec time than after tuning has started against it.
- **`level-design`** -- fires when the spec describes playable space. Owns whether character metrics are locked before layout is committed (a layout built on TBD jump distance gets rebuilt) and whether pacing and guidance are specified rather than assumed.
- **`game-engines`** -- fires when the spec commits to an engine, a platform set, or a monetization model. Owns licence ceilings and deployment gates -- console access and free-tier revenue thresholds are the constraints most often discovered after they bind.
- **`game-art-pipeline`** -- fires when the spec implies an asset pipeline. Owns the source-of-truth decision, the material-ownership question, and whether validation exists, all of which are cheapest to settle before assets multiply.
- **`ai-3d-integration`** -- fires when the spec puts a model in control of a creative tool. Owns what gets verified versus assumed, and where the recovery boundary is for destructive operations. Specs in this space routinely settle the tool surface and leave both open.
- **`architectural-design`** / **`interior-and-spatial`** / **`architectural-tooling`** -- fire only for built-environment projects, not software architecture.
...
Other agents (`otel-instrumentation`, `otel-pipeline`, `fp-effects`, `fp-verification`, `oo-patterns`, `oo-architecture`, domain specialists like `pdf` / `citation-and-bibliography` / `scholarly-publishing` / `copyright-and-permissions` / `print-production` / `graphics-programming` / `audio-programming` / `text-engineering`, language specialists like `rust-async` / `typescript-types`) can be consulted opportunistically during the grill when a tactical question lands in their lens. Do not include them in the default critique fan-out.
