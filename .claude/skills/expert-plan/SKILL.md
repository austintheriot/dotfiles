...
Other agents (`otel-instrumentation`, `otel-pipeline`, `fp-effects`, `fp-verification`, `oo-patterns`, `oo-architecture`, domain specialists like `pdf` / `graphics-programming` / `audio-programming`, language specialists like `rust-async` / `typescript-types`) can be consulted opportunistically during the grill when a tactical question lands in their lens. Do not include them in the default critique fan-out.
