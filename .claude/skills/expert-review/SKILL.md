...
- **Design and user experience**: `interaction-design`, `visual-hierarchy`, `design-systems`, `ai-interface-design`, `information-architecture`, `content-design`, `expert-user-efficiency`. **`interaction-design` and `expert-user-efficiency` disagree by construction** -- the first argues novice-first defaults, the second argues for the practiced daily user of a professional tool. On a high-frequency working surface, dispatch both and let the synthesis carry the disagreement rather than averaging it.
- **Cross-cutting**: `bug-hunter`, `code-simplifier`, `test-coverage`, `readability`, `debuggability`, `documentation`, `security`, `performance`, `accessibility`, `api-design`, `concurrency`, `i18n`, `text-engineering`, `ci-pipeline`, `web-analytics`, `graphics-programming`, `audio-programming`, `pdf`, `webassembly`, `browser-spec`, `mobile-native`, `sync-and-offline`, `platform-payments`, `platform-release`, `desktop-native`, `native-bridge`, `input-and-peripherals`, `app-privacy-compliance`, `licensing-and-oss`, `build-systems`, `crash-and-release-health`, `llm-app`, `ai-creative-tooling`, `neovim`, `devops-infrastructure`, `data-flow`, `first-principles`
...
| `platform-payments` | monetization and entitlement code: StoreKit (`Product`, `Transaction`, `SKPaymentQueue`, `AppStore.sync()`, `SubscriptionStoreView`, `.storekit` files); Play Billing (`BillingClient`, `queryProductDetailsAsync`, `launchBillingFlow`, `acknowledgePurchase`, `consumeAsync`, `PendingPurchasesParams`); Stripe or other processor subscription / invoice / PaymentIntent code; App Store Server Notification, Play real-time developer notification, and Stripe webhook handlers; entitlement services, subscription state machines, paywall gating and purchase-keyed feature flags; receipt or transaction validation on device or server; vendor integration (RevenueCat, Adapty, Superwall, Qonversion); restore-purchases and purchase-to-account linking; product catalog / offer config encoding eligibility. Skip pricing strategy and paywall copy (route to `product-leadership`), payment code with no store or subscription surface, and store submission mechanics unrelated to payments (route to `platform-release`) |
| `platform-release` | release / distribution code: signing scripts and config (`codesign`, `signtool`, `apksigner`, `jarsigner`, keystores, certificates, entitlements, provisioning profiles); notarization and stapling (`notarytool`, `stapler`, `altool`); export and packaging (`xcodebuild -exportArchive`, `ExportOptions.plist`, `.pkg` / `.dmg`, `makeappx`, WiX, Inno Setup, NSIS, MSIX manifests); store submission automation (App Store Connect API, `deliver` / `supply`, Play Publishing API, release tracks, staged / phased rollout settings); fastlane lanes; version and build-number computation; privacy manifests and capability declarations as submission gates; auto-update (Sparkle appcasts and keys, electron-updater / electron-builder, Electron fuses, Tauri updater, Velopack, WinSparkle); Linux packaging and repo config (`.deb` / `.rpm` signing, `.sources`, `gpgcheck`, Flatpak manifests, AppImage); kill switches, force-update gates, minimum-version checks. Skip CI workflow structure / runners / secrets (route to `ci-pipeline`), post-launch runtime behavior (route to `mobile-native` / `desktop-native`), payment mechanics (route to `platform-payments`), and build-graph concerns (route to `build-systems`) |
| `desktop-native` | desktop runtime code for Windows / macOS / Linux: windowing (creation, sizing, positioning, levels, collection behavior, multi-monitor, state restoration, DPI handling, custom title bars, drag regions); application lifecycle (file / URL activation, single-instance enforcement, quit and session-end handling, sleep and power, crash recovery); OS integration (menus, global hotkeys, tray / status items, notifications, clipboard, drag and drop, file associations, protocol handlers, startup registration, jump lists); filesystem (per-platform data locations, atomic replace, filesystem watching, reserved names, sandbox access, security-scoped bookmarks, TCC surfaces); desktop UX (shortcuts, text-editing bindings, menu structure, document model, printing, theming); long-uptime resource behavior; platform manifests and entitlements as runtime gates. Skip iOS / Android (route to `mobile-native`), signing / notarization / installers / update delivery (route to `platform-release`), Electron and Tauri IPC internals (route to `native-bridge`), and peripheral access (route to `input-and-peripherals`) |
| `app-privacy-compliance` | privacy / regulatory surface: consent banners, gates, preference centers, consent-state storage, universal opt-out signal handling; analytics / advertising / attribution / session-replay SDK initialization and config; personal-data collection (new user-record fields, forms, device identifiers, location, biometrics, health-adjacent data, contacts, photos); data-subject request handling (export, deletion, access); retention and deletion jobs, TTL config; logging / crash-reporting config where personal data can reach it; `PrivacyInfo.xcprivacy`, App Privacy label source, Play Data Safety declarations; third-party SDK additions in a manifest or lockfile; code sending user content to a model vendor; age gates, parental consent, kids-mode; privacy policy text in the repo checked against the code. Skip attacker-oriented threat modeling (route to `security`), event taxonomy and funnel correctness (route to `web-analytics`), and submission mechanics beyond privacy artifacts (route to `platform-release`) |
| `licensing-and-oss` | licensing and OSS compliance: dependency manifests and lockfiles where a dependency is added, removed, or version-bumped; license policy config (`deny.toml`, allowlists, CI license gates); SBOM generation and consumption; `LICENSE` / `NOTICE` / `COPYING` and attribution surfaces including in-app about screens and license pages; vendored or copy-pasted third-party code and code with unclear provenance; font files and webfont pipelines especially any that subset; non-code assets under Creative Commons terms; `CONTRIBUTING.md`, CLA bots, DCO config; build config determining linking mode where weak copyleft is involved; any dependency that changed license between versions. Skip supply-chain security -- typosquatting, malicious packages, compromised maintainers (route to `security`), store submission mechanics (route to `platform-release`), dependency resolution as a build concern (route to `build-systems`), and notice prose quality (route to `documentation`) |
| `build-systems` | the build graph beneath CI: build definitions (`build.gradle`(`.kts`), `BUILD`/`MODULE.bazel`, `CMakeLists.txt`/`CMakePresets.json`, `Makefile`, `build.ninja`, `Cargo.toml`/`.cargo/config.toml`/`build.rs`, `*.csproj`/`Directory.Build.props`, package.json build scripts, `turbo.json`/`nx.json`, `BUCK`, `pants.toml`, `flake.nix`); custom tasks / script phases / codegen steps and their declared inputs and outputs; cache configuration including the read-vs-write permission split; toolchain pin files and wrappers; lockfiles and the commands CI uses to install from them; dependency additions and bumps as a resolution concern; cross-compilation config; reproducibility settings (timestamps, path mapping, archive creation, strip and debug-info). Skip CI workflow structure / runners / secrets / gating (route to `ci-pipeline`, naming the shared cache-trust and invocation seams), artifact signing and distribution (route to `platform-release`), dependency license obligations (route to `licensing-and-oss`), and runtime performance of the built software (route to `performance`) |
| `crash-and-release-health` | crash capture and client release telemetry: crash-reporting SDK init and config (sampling, pre-send hooks, feature toggles); uncaught-exception handlers, signal handlers, panic hooks, unhandled-rejection listeners; error boundaries where reporting is the concern; symbol generation and upload (debug-info settings, stripping, split debug info, mapping-file retention, CI upload steps); source-map generation / hosting / upload; release-health alerting and staged-rollout gating logic; breadcrumb, context, and user-identification config; native crash handling in cross-platform apps; third-party SDK integration whose crashes would count against your metrics. Skip server SLOs and on-call practice (route to `observability-practice`), dev-time debugging affordances (route to `debuggability`), rollout mechanics (route to `platform-release` -- this lens owns the metric that says halt), and whether payload data is lawful (route to `app-privacy-compliance`) |
| `native-bridge` | the host-to-native boundary: Electron main-process code, preload scripts, `contextBridge` exposure, `ipcMain` handlers, `webPreferences`, fuses, permission handlers; Tauri commands, `tauri.conf.json`, `capabilities/`, custom protocol handlers; React Native native modules and TurboModules, Codegen specs, JSI / C++ bridging, Expo modules; Flutter platform channels, Pigeon definitions, plugin registration, `dart:ffi`; KMP `expect` / `actual`, cinterop, Swift Export; Capacitor and Cordova plugin authoring; WebView embedding in native apps (`WKScriptMessageHandler`, `evaluateJavaScript`, `addWebMessageListener`, `addJavascriptInterface`, asset loaders, file-access settings); native module build config where ABI compatibility is at stake; OTA update config interacting with native compatibility. Skip platform runtime behavior (route to `desktop-native` / `mobile-native`), capability semantics and permission models (route to `input-and-peripherals`), Rust FFI and WASM boundaries (route to `rust-ffi` / `webassembly`), and delivery mechanics (route to `platform-release`) |
| `input-and-peripherals` | input handling and device access: pointer / touch / mouse / stylus event handling, drawing and annotation surfaces, gesture recognition and conflict, hit testing; keyboard handling, shortcuts, text input and composition / IME; gamepad and controller input; camera, photo-library, and microphone access and their pickers; Bluetooth and BLE scanning, connection, characteristic access; location (precise vs coarse, foreground vs background); file access (pickers, security-scoped bookmarks, persisted grants, drag and drop); USB / HID / serial / MIDI; biometrics and secure-storage access control; sensors, haptics, NFC, screen capture, clipboard where device access is the concern; permission declaration files (`Info.plist` usage strings, `AndroidManifest.xml` permissions, entitlements, permissions-policy) and request sites. Skip platform-primitive reinvention (route to `browser-spec`), bridge mechanics (route to `native-bridge`), assistive technology (route to `accessibility`), rendering the stroke (route to `graphics-programming`), audio processing after capture (route to `audio-programming`), and lawfulness of collection (route to `app-privacy-compliance`) |
...
| `interaction-design` | rendered UI with behaviour: components and templates, form handling and validation code, state machines behind a view, dialog / modal / drawer components, toast and notification code, loading and error boundaries, optimistic-update logic, animation and transition code, empty-state components, destructive-action handlers, consent / subscription / cancellation flows. Also design artifacts: mockups, prototypes, `.html` with inline styles, Storybook stories. Skip backend / CLI / config with no rendered surface |
| `visual-hierarchy` | presentation code: stylesheets, CSS-in-JS, inline `style` attributes, Tailwind class strings, theme / token / palette files, component markup where layout is expressed (flex / grid / spacing), SwiftUI and Compose layout, table and data-grid rendering, chart styling outside the plot, typography config, dark-mode definitions. Also design artifacts. Skip generated or vendored stylesheets |
| `design-systems` | the system's own surface: token files (`tokens.json`, `*.tokens.json`, DTCG files, Style Dictionary config, Tokens Studio exports), theme and CSS custom-property definitions, `tailwind.config.*` theme sections, shared component definitions and prop types, component barrel exports, Storybook stories, visual-regression config. Also any diff that adds a component, adds a variant, adds a colour / spacing / radius value, or changes a shared component's rendered DOM or ARIA role. Skip one-off surfaces the system was never claimed to govern |
| `ai-interface-design` | surfaces where model output reaches a person: suggestion / completion / autocomplete interfaces, review or triage queues over machine output, chat and assistant surfaces, streaming response rendering, agent action logs and approval gates, citation and source display, confidence or score display, feedback controls (thumbs, flag, correct), accept / reject / edit affordances over generated content, and the provenance columns that determine what those surfaces can show. Skip backend model plumbing with no human surface (route to `llm-app`) |
| `information-architecture` | structure and naming: route definitions, navigation / menu / sidebar / tab components, breadcrumb code, page and section titles, search implementation and its scope, filter and facet code, URL construction and parsing, slug generation, redirect maps, sitemap generation, glossary or terminology files, and any user-facing string naming a category or destination. Also any diff adding a page, adding a top-level nav item, or changing what a URL encodes. Skip products too small to have a structure problem |
| `content-design` | user-facing strings: message and string catalogs, translation source files, error and validation messages, empty-state copy, confirmation and dialog text, notification and toast strings, form labels and helper text, tooltip content, onboarding sequences, and product-sent email templates. Skip code comments, logs, commit messages, and documentation surfaces (route to `documentation`) |
| `expert-user-efficiency` | high-frequency interfaces only: review and triage queues, data grids with per-row actions, editors, operational dashboards, admin and moderation tools, interactive CLI surfaces, keyboard-shortcut and command-palette code, selection and bulk-operation logic, and any workflow the code or spec shows is repeated many times per session. **Requires evidence of repeated use.** Skip signup, onboarding, invitation, recovery, and annual-configuration surfaces entirely (route to `interaction-design`) |
| `blender-3d` | Blender and 3D authoring: `.blend` files; Python touching `bpy` / `bmesh` / `mathutils` (add-ons, exporters, batch scripts); modeling, retopology, UV, and bake work; material and shader-node setup, texture assignment, color-space configuration; rigging and skinning (armatures, weights, constraints, Rigify, actions); scene organization (collections, linking, library overrides, asset catalogs); exporter settings authored in Blender; bake and view-transform configuration. Skip the cross-boundary contract itself (route to `game-art-pipeline`), renderer internals (route to `graphics-programming`), and model-driven Blender automation (route to `ai-3d-integration`) |
| `game-art-pipeline` | the authored-content-to-engine boundary: import/export configuration (Unity importer presets and `AssetPostprocessor`, Unreal import settings and Interchange pipeline stacks, Godot `.import` files); interchange format choice and conversion (glTF, FBX, USD, Alembic); texture compression and per-platform overrides, channel packing, KTX2/Basis; collision, LOD, and lightmap-UV conventions; asset naming conventions that downstream tools bind to; binary version control (`.gitignore`, `.gitattributes`, LFS setup and locking, Perforce typemap, `.meta` file handling); asset validation code (Unreal Data Validation, Unity postprocessor checks, glTF-Validator in CI); DDC and import-cache configuration. Skip DCC-internal authoring (route to `blender-3d`), engine selection (route to `game-engines`), and the build graph itself (route to `build-systems`) |
| `level-design` | spatial gameplay: level layouts, floor plans, blockouts, greybox geometry, world maps; encounter design (enemy placement, cover, sightlines, entries and exits); pacing documents and intensity curves; player guidance (lighting as direction, landmarks, framing, breadcrumbing, waypoint policy); traversal affordance, verticality, shortcuts and loops; open-world region structure and point-of-interest density; level design documents and specs. Skip rules and systems (route to `game-mechanics`), real buildings (route to `architectural-design`), asset authoring (route to `blender-3d` / `game-art-pipeline`), and lighting as a rendering concern (route to `graphics-programming`) |
| `game-engines` | engine and tool selection, licensing, and deployment: engine choice or reconsideration; licence, royalty, seat-cost, revenue-threshold, and free-tier-ceiling questions; splash-screen and attribution requirements; console access paths and porting partners; web export constraints (WASM, scripting-language gaps, SharedArrayBuffer/COOP-COEP, size limits); mobile store requirements as they constrain engine and plugin choice; middleware licensing (FMOD, Wwise) and asset-store terms; migration, porting, and lock-in assessment; architecture for insulating game logic from an engine; major engine-version transition planning. Skip gameplay systems (route to `game-mechanics`), asset pipeline (route to `game-art-pipeline`), and submission mechanics (route to `platform-release`) |
| `game-mechanics` | gameplay systems design: core loops, mechanics, verbs, win/loss conditions; progression and reward structure (curves, unlocks, levelling, pacing); economy design (currencies, sinks and faucets, markets, crafting, drop tables); randomness (drop rates, crit models, distribution vs perceived fairness); difficulty tuning, options, and dynamic adjustment; game feel (input buffering, coyote time, hit stop, cancel windows, frame data); monetization design as an incentive question (F2P loops, gacha, battle passes) and its dark-pattern surface; playtest design and the telemetry that answers a design question; procedural generation as a content strategy. Skip engine and tooling (route to `game-engines`), spatial layout (route to `level-design`), UI and onboarding flows (route to `interaction-design`), and analytics implementation (route to `web-analytics`) |
| `ai-3d-integration` | model-driven creative tooling: MCP server or client code for a DCC application (Blender, Maya, Houdini, Unreal, Unity, Figma); code that generates or executes DCC scripts from model output (`bpy`, Maya Python, Houdini Python, engine editor scripting); perception loops (viewport or camera renders fed back to a model, screenshot tools, vision-model verification); tool definitions exposed to a model that mutate creative state; approval gating, sandboxing, and human-in-the-loop checkpoints around destructive creative operations; generated-asset integration (text-to-3D, image-to-3D, AI texture or animation generation) and its provenance and licensing metadata; asset-marketplace integrations whose results re-enter model context. Skip general LLM application concerns (route to `llm-app`), Blender authoring technique as such (route to `blender-3d`), and ideation practice (route to `ai-creative-tooling`) |
| `ai-creative-tooling` | AI in creative production as a practice: where generative tooling is used in a content workflow (concept and moodboarding, placeholder assets, dialogue, localization drafts, PCG assistance, playtest bots, QA triage); AI NPC and runtime-dialogue systems and their guardrail, latency, and cost surface; generated-content provenance policy and disclosure obligations (platform AI-disclosure rules, store policies); AI-generated asset licensing and copyright posture; team policy documents about generative-tool use; evaluation of creative output where no unit test exists. Skip the mechanics of driving a tool through MCP (route to `ai-3d-integration`), general LLM application code (route to `llm-app`), and consent and data-collection lawfulness (route to `app-privacy-compliance`) |
| `neovim` | Neovim and Vim: `init.lua` / `init.vim` and any `nvim/` config tree (`lua/`, `plugin/`, `after/`, `ftplugin/`, `lsp/`, `colors/`, `queries/`); Lua calling `vim.*` (plugin source, config modules, autocmd callbacks, user commands); plugin-manager specs (lazy.nvim, `vim.pack`, mini.deps, rocks.nvim, packer, vim-plug); LSP configuration (`vim.lsp.config` / `vim.lsp.enable`, `lsp/<name>.lua`, lspconfig, capabilities, `LspAttach`, formatting ownership, `vim.diagnostic` config); treesitter config and custom queries; keymaps, autocmds, options; agent-editor integration (RPC servers, `--listen` / `--server` / `--remote-*`, terminal-embedded agents, buffer reload on external edit); headless automation and test harnesses (plenary busted, mini.test, vusted); Vimscript in a Neovim context. Skip general Lua quality with no `vim.*` surface (route to `readability`) |
| `architectural-design` | building architecture (NOT software architecture): design theory and proportional systems; historical movements and their arguments; architectural criticism and urbanism (Jacobs, Lynch, Alexander, Gehl, New Urbanism, Space Syntax); practice structure (RIBA Plan of Work, AIA phases, delivery methods, discipline coordination); building codes and life safety as design constraints; accessibility law and universal design; sustainability frameworks (LEED, BREEAM, Passivhaus, WELL) and embodied-carbon assessment; mass timber, retrofit-first, and adaptive reuse; art in architecture and site-specificity; architectural acoustics and soundscape; environmental graphic design, signage, and wayfinding typography. Skip playable space (route to `level-design`), interior fit-out and inhabited-room concerns (route to `interior-and-spatial`), and CAD/BIM software (route to `architectural-tooling`) |
| `interior-and-spatial` | interiors and the inhabited room: spatial planning, adjacency, and circulation at room scale; anthropometrics and ergonomics; lighting design (illuminance targets by task, colour temperature, CRI/TM-30, layered lighting, daylight, glare, circadian claims); room acoustics (reverberation time, absorption vs diffusion, NRC/STC ratings, open-plan acoustics); colour and material selection (durability, maintenance, VOC and Red List concerns); furniture specification and FF&E; biophilic design and its evidence base; interior wayfinding and signage at room scale; workplace, residential, and hospitality interior programme. Skip building-scale design and theory (route to `architectural-design`), playable space (route to `level-design`), and specification software (route to `architectural-tooling`) |
| `architectural-tooling` | architectural software and digital deliverables: CAD and BIM authoring (Revit, ArchiCAD, Vectorworks, Allplan, BricsCAD, FreeCAD BIM); openBIM and interoperability (IFC, buildingSMART, COBie, BCF) and LOD/LOIN definitions; clash detection and model coordination (Navisworks, Solibri); parametric and computational design (Rhino, Grasshopper, Rhino.Inside.Revit, Dynamo); visualization and rendering pipelines for architecture (V-Ray, Corona, Enscape, Lumion, Twinmotion, D5, Unreal for archviz); reality capture and scan-to-BIM (LiDAR, photogrammetry, point clouds); drawing standards, sheet setup, and documentation conventions; material and texture libraries and their licensing; AI image tools in architectural concept work. Skip design theory (route to `architectural-design`), interior specification (route to `interior-and-spatial`), and game asset pipelines (route to `game-art-pipeline`) |
