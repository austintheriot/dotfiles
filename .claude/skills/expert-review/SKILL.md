...
- `readability` -- naming, function shape, flow/paragraphing, comment quality, abstraction altitude, API surface clarity. Grounded in the user's `coding-style.md` and the established readability canon (Ousterhout, Beck, Kernighan/Plauger, Metz, CUPID). Reads project style guides; project conventions win. Distinct from `code-simplifier` -- this lens is "even at appropriate complexity, can a human follow this?" `~/.claude/agents/readability.md`.
- `debuggability` -- runtime observability for *developers*: error context, panic / unwrap reachability, stack-trace quality, source-map / debug-symbol availability, request-id propagation, log levels, hidden state, reproducibility, async stack visibility, presence of standard per-environment debug tooling (Node `--inspect` / `AsyncLocalStorage` / `diagnostics_channel`; browser DevTools / source maps / framework hooks; Rust `tracing` / `RUST_BACKTRACE` / `#[track_caller]` / `tokio-console`; Rust+WASM `console_error_panic_hook` / DWARF). Distinct from `observability-practice` (production telemetry, SLOs, sampling) -- this lens is "future-you can answer 'what is this doing right now' in seconds, not minutes." `~/.claude/agents/debuggability.md`.
- `documentation` -- doc-comment quality at API boundaries, doctest / example presence, stale-doc detection (doc claim vs. code reality), per-language conventions (rustdoc `# Safety` / `# Panics` / `# Errors` / intra-doc links / `#[deprecated(note)]`; TypeScript JSDoc/TSDoc with `{@link}`; Python Google/NumPy/reST; Go godoc identifier-first / `Deprecated:` prefix / Example functions), and out-of-code surfaces (README, CHANGELOG with Keep-a-Changelog format, ADRs, runbooks, migration guides, examples directories). Grounded in Diátaxis (tutorial / how-to / reference / explanation separation), Write the Docs principles, Google's developer doc style guide, "document why not what." Distinct from `readability` (which covers in-code naming and function shape) -- this lens is "is the documentation correct, complete, and in the right shape?" `~/.claude/agents/documentation.md`.
...
| Any human-read code (most code): naming, function shape, paragraphing, comments, abstraction altitude, API surface. Skip only for purely-generated bindings, lock files, or other code humans rarely read | readability |
| Code with runtime concerns: error handling, panics / unwraps, async / concurrent work, stateful modules, build/bundler configs, browser code, server code, WebAssembly. Skip only for purely pure / static / generated content (e.g., type-only files, lock files, fixture data) | debuggability |
| Public API surfaces (anything `pub`/`export`/exported), doc comments, README / CHANGELOG / ADR / runbook / migration-guide / examples-directory files, or any `*.md` documentation. Also: every code review where the diff changes a signature (does the doc still match?) or adds a public item (is it documented?). Skip only for purely-internal / private-helper code with no doc-comment expectations | documentation |
