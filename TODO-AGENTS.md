...
- Three pre-existing container failures, newly visible because cargo had
  never run in that image:
    ...
    - `tmux_tools::conf_split::the_shared_half_of_the_split_survives` was a
      **genuine Docker-only defect**, exactly the class the container leg
      exists to catch, and is FIXED. `.config/tmux/tmux.conf` ends with
      `run '~/.tmux/plugins/tpm/tpm'`, and the image never installed tpm at
      all, so `source-file` exited 127 with empty stderr. An earlier note
      here said the image kept plugins at `~/.config/tmux/plugins/tpm`; that
      was wrong. The config is internally consistent, and the image simply
      lacked the dependency `deps.toml` declares.
