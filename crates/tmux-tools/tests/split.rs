...
fn a_recognized_layout_exits_zero_and_splits() {
    let server = Server::new("split-known");
...
fn an_unrecognized_layout_splits_nothing() {
    let server = Server::new("split-unknown");
    server.tmux(&["new-session", "-d", "-s", "my-feature-branch"]);

    let before = server.tmux(&["list-panes", "-t", "my-feature-branch:1", "-F", "#{pane_id}"]);
    ...
    let after = server.tmux(&["list-panes", "-t", "my-feature-branch:1", "-F", "#{pane_id}"]);
...
fn the_editor_layout_focuses_the_editor_pane() {
    let server = Server::new("split-editor");
    server.tmux(&["new-session", "-d", "-s", "probe"]);
    ...
    let active = server.tmux(&["display-message", "-p", "-t", "probe:1", "#{pane_id}"]);
    assert_eq!(
